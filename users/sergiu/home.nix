...

{
  ...
  home.shellAliases = {
    ...
    update = "cd ~/NixOS && ${pkgs.time}/bin/time -f 'Duration: %E' sudo bash -c 'nix flake update && nixos-rebuild boot --flake .#$(hostname)' && git add flake.lock && git commit -m 'update' && git push";
