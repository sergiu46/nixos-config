{ pkgs, stateVersion, ... }:
{
  ...
  home.shellAliases = {
    ...
    mount-portable = "sudo mount -t f2fs -o noatime,lazytime,background_gc=on,compress_algorithm=zstd:6,compress_chksum,compress_mode=user,atgc,gc_merge,flush_merge,checkpoint_merge,inline_xattr /dev/disk/by-label/NIX-ROOT /mnt && sudo chattr +c /mnt && sudo mkdir -p /mnt/boot && sudo mount /dev/disk/by-label/NIX-BOOT /mnt/boot";
